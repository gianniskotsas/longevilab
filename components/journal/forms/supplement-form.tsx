...
export function SupplementForm({ onSuccess, onCancel }: SupplementFormProps) {
  ...
  const createSupplement = trpc.medications.createSupplement.useMutation({
    onSuccess: () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      setError("Please enter the supplement name");
      return;
    }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <Field>
        <FieldLabel>Supplement Name</FieldLabel>
        <FieldContent>
          <Input
            type="text"
      ...
      <Field>
        ...
        <FieldContent>
          <Input
            type="text"
            placeholder="e.g., 1000 IU softgel"
            value={dosage}
      ...
      <div className="flex gap-3 pt-4">
        ...
        <Button type="submit" className="flex-1" disabled={createSupplement.isPending}>
          {createSupplement.isPending ? "Saving..." : "Add Supplement"}
        </Button>
      </div>
    </form>
  );
